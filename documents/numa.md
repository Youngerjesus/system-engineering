...
NUMA 아키텍처의 반대는 UMA (Uniform Memory Access) 로  공용 BUS 를 통해서 메모리에 접근하는 방식이다.
...
`numactl --show` 를 통해서 NUMA 정책을 볼 수 있다.
...
- 두 번째는 bind 정책으로 특정 프로세스를 특정 노드에 바인딩시키는 방식을 취한다. 특정 노드에 할당시켜서 거기에 있는 메모리를 받도록 하는 방식이다.
...
이 경우에 조심해야 할 것은 하나의 노드에만 집중적으로 하기 때문에 메모리 불균형이 생길 수 있다는 점이다.
...
zone 에 대한 정보는 `/proc/buddyinfo` 에서 살펴볼 수 있다.

```bash
ubuntu@ip-192-168-111-182:~$ cat /proc/buddyinfo
Node 0, zone      DMA      1      0      0      1      2      1      1      1      0      1      3
Node 0, zone    DMA32   3428   2965   1761    328    115     13      5      1      4     22    393
...
- Normal 은 커널과 프로세스가 메모리를 필요로 할 때 요청하는 영역이다. 즉 Node 0 과 Node 1 이 있고 Node 0 에서 Normal 의 영역이 부족하다면 Node 1 의 Normal 에게 요청하는 매커니즘이다.

이제 vm.zone_reclaim_mode 에 대해서 보자. 이 값은 총 4 개의 값을 가질 수 있지만 실제적으로 중요한 값은 0 과 1 이다.

- 0 은 disable 을 의미하며 해당 Zone 안에서 재할당을 하지 않겠다는 의미다. 즉 메모리가 부족하면 다른 Zone 에서 가져와서 사용하겠다느 뜻이다.
- 1 은 enable 을 말하며 Zone 안에서 재할당을 하겠다는 뜻이다. 해당 Zone 에서 메모리가 부족하면 재할당을 하기 위해서 필요없는 메모리를 제거하고 재사용해보고 그래도 부족하다면 다른 Zone 에서 메모리를 할당 받아서 사용한다.
...
이것은 정답이 없는 문제이기도 해서 그냥 이런 케이스로 사용할 수 있다 정도만을 알아두자.

NUMA 시스템에서 워크로드를 확인할 때 고려해봐야 할 것이 프로세스가 노드의 메모리를 뛰어 넘는지, 그리고 프로세스가 여러 CPU 가 필요한 멀티 스레드인지, 싱글 스레드인지가 중요하다.
